...
Star::Star(int rad, sf::Color clr, sf::Vector2f pos) : Figure(rad * 2, rad * 2, clr, pos)
{
	radius = rad;
	vertex_count = 10;
	initialize_vertices();
...
{
	return "5-" + std::to_string(radius) + "-" + std::to_string(default_color.r) + "," + std::to_string(default_color.g) + "," + std::to_string(default_color.b)
...
void Star::initialize_vertices()
{
	...
	for (int i = 0; i < vertex_count; i += 2)
	{
		...
		vertices[i] = sf::Vertex(position + sf::Vector2f(offset.x / 2, offset.y / 2), color);
		deg -= 72;
	}
	deg = 54;
