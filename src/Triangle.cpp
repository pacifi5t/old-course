...
Triangle::Triangle(int w, int h, sf::Color clr, sf::Vector2f pos)
    : Figure(h, w, clr, pos) {
  vertex_count = 3;
  initialize_vertices();
}

Figure *Triangle::clone() { return new Triangle(*this); }
...
std::string Triangle::to_string() {
  return "3-" + std::to_string(width) + "-" + std::to_string(height) + "-" +
...
void Triangle::initialize_vertices() {
  ...
  vertices[0] =
      sf::Vertex(sf::Vector2f(position.x, position.y - height / 2), color);
  vertices[1] = sf::Vertex(
      sf::Vector2f(position.x + width / 2, position.y + height / 2),
      color);
