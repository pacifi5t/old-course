...
void Figure::initialize_vertices()
{
	vertices = nullptr;
}
...
bool Figure::crossed_border()
{
	...
	if (position.x - width / 2 < 0 || position.x + width / 2 > window_size.x || position.y - height / 2 < 0 || position.y + height / 2 > window_size.y)
		return true;
	else return false;
}
...
{
	if (m_pos.x < position.x + width / 2 && m_pos.x > position.x - width / 2 && m_pos.y < position.y + height / 2 && m_pos.y > position.y - height / 2)
		return true;
	else
		return false;
}
...
bool Figure::intersects(Figure* other)
{
	...
	sf::Vector2f delta = sf::Vector2f(position.x - other->position.x, position.y - other->position.y);
	sf::Vector2f distance = sf::Vector2f(fabs(delta.x) - (other->width / 2 + width / 2), fabs(delta.y) - (other->height / 2 + height / 2));
...
{
	sf::Vector2f offset(ofs.x * cos(deg * 3.14 / 180), ofs.y * sin(deg * 3.14 / 180));
	move(offset);
	if (crossed_border())
		move(-offset);
}
...
int Figure::get_vertex_count()
{
	return vertex_count;
}

void Figure::get_memento(Memento& mem)
{
	from_string(mem.state);
}
...
void Figure::set_selection()
{
	is_selected = true;
}
void Figure::set_memento(Memento& mem)
{
	mem.state = to_string();
}
