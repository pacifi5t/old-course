...
Ellipse::Ellipse(int w, int h, sf::Color clr, sf::Vector2f pos) : Figure(h, w, clr, pos)
{
	vertex_count = 36;
	initialize_vertices();
}
...
{
	return "2-" + std::to_string(width) + "-" + std::to_string(height) + "-" + std::to_string(default_color.r) + "," + std::to_string(default_color.g) + "," + std::to_string(default_color.b)
