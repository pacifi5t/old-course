...
void Programm::handle_input(Viewport* view, std::vector<Figure*>& shapes, sf::Vector2f& offset)
{
	offset -= offset;
	sf::Event event;
	...
	while (view->get_window()->pollEvent(event))
	{
		...
		if (event.type == sf::Event::MouseButtonPressed)
			switch (code)
			{
			case sf::Mouse::Left: select_shape(shapes, sf::Mouse::getPosition() - sf::Vector2i(8, 32)); break;
			case sf::Mouse::Right: deselect_shape(shapes, sf::Mouse::getPosition() - sf::Vector2i(8, 32)); break;
			}
		if (event.type == sf::Event::KeyPressed)
		{
			switch (code) 
			{
			case sf::Keyboard::V: change_visibility(shapes); break;
			case sf::Keyboard::D: deform(shapes); break;
			case sf::Keyboard::S: save_checkpoint(shapes); break;
			case sf::Keyboard::L: load_checkpoint(shapes); break;
			case sf::Keyboard::Z: load_start(shapes); break;
			case sf::Keyboard::T: trail_enabled = !trail_enabled; std::cout << "Trail switched\n"; break;
			case sf::Keyboard::C: add_to_composite(shapes); break;
			case sf::Keyboard::B: build_new_figure(shapes); break;
			}
		}
	}
}
...
void Programm::select_shape(std::vector<Figure*>& shapes, sf::Vector2i mouse_position)
{
	for (auto& elem : shapes)
	{
...
void Programm::deselect_shape(std::vector<Figure*>& shapes, sf::Vector2i mouse_position)
{
	int i = 0;
	for (auto& elem : shapes)
	{
		if (elem->hover(mouse_position))
		{
			//Figure* temp = elem; - triggers expection
			shapes.erase(shapes.begin() + i);
			//delete temp; - triggers expection
